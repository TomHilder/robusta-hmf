...
\begin{abstract}
  ...
  It works by optimizing a likelihood-like objective function that is adaptive to bad data and has a probabilistic interpretation.
    We deliver a fast JAX \hogg{JAX, Jax, or jax?} implementation, Robusta-HMF, and practical guidance for users.
...
\end{align}
\hogg{note my addition of thin spaces in the above}
...
\begin{enumerate}
    ...
    \item \textbf{Rectangular data}: At each fixed pixel index $j$, pixel values across all spectra $i$ correspond to the same feature or (in the case of spectroscopy) wavelength. \hogg{I think we should talk about objects and features, and this property here is rectangularity. There is additionally an assumption that $M$ is the same for all $i$, where missing data are accounted for by zero-padding the weights.}
    \item \textbf{Basis orthogonality}: The inferred basis vectors, and so the rows of $G$, will be made strictly orthogonal and ordered by explained variance, in analogy to principal component analysis.
      \item \textbf{Low rank}: The rank of the model is restricted to $1 \le K < \min(N, M)$. \hogg{I believe we must be \emph{strictly} low rank to have nontrivial behavior.}
...
\hogg{Does this paragraph capture what you wanted to say in the ``clunky'' paragraph it replaces?}
...
\software{
  Jax \citep{jax} \hogg{how is Jax typeset officially? You have all-caps in the abstract.}
  \textsl{numpy} \cite{numpy}
  \hogg{what else?}
}

\hogg{I generally typeset projects and codebases with textsl, but I don't feel strongly, so \textsl{APOGEE} and \textsl{Jax}.}
