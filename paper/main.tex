...
\newcommand{\hogg}[1]{\textcolor{teal}{\textbf{DWH says:} #1}}
...
\title{Robust Heteroskedastic Matrix Factorization: \\
  ...
  \hogg{love the 19$^{\mathrm{th}}$\,c. title}}
...
The $K$ \emph{components} delivered by PCA are orthonormal basis vectors contained in the $K$ rows of some $K \times M$ matrix ${\bf G}$, while the \emph{coefficients} are the $N$ columns of some $N \times K$ matrix ${\bf A}$, such that ${\bf L} = {\bf A}\,{\bf G}$.
\hogg{I reversed rows and columns in the previous sentence; am I wrong?}
