...
For each fit, we initialized with an SVD, and iterated the algorithm until the maximum fractional change in any entry of ${\bf G}$ was less than $10^{-2}$ \tom{this is maybe a bit low, but it doesn't seem to matter much. Happy to redo it before submission if desired}, checking for convergence every 10 cycles.
