"""
Train RHMF models on the FULL Gaia RVS main-sequence sample (not binned),
over a (K, Q) grid, for the referee-requested full-dataset + CV analysis.

The sample is the union of all 14 main-sequence bins from gaia_config.py, so
the selection is identical to the per-bin analysis in the paper. Everything
else (edge clipping, train/test split seed, NaN handling, model settings)
mirrors train_bins.py exactly. State files use the per-bin naming convention
with bin tag "full_ms", so all analysis_funcs helpers work unchanged.
...
Run analyse_full_ms.py afterwards for CV scores, best-model selection, and
...
BIN_TAG = "full_ms"  # used in state filenames: converged_state_R{K}_Q{Q}_bin_full_ms.npz

# ============================================================================ #


def build_full_ms_sample():
    """Union of all main-sequence bins: unique data indices + source ids.

    Bins overlap (widths exceed spacing), so duplicates are removed while
    keeping idx/ids aligned.
    """
    ...
    return data, all_idx[first], all_ids[first]
...
def main(ranks, q_vals, shard, n_shards, max_iter=MAX_ITER, results_dir=RESULTS_DIR):
    print("Building full main-sequence sample (union of all bins)...")
    ...
    for Q, rank in tqdm(my_grid, desc=f"full_ms shard {shard}"):
        state_file = results_dir / f"converged_state_R{rank}_Q{Q:.2f}_bin_{BIN_TAG}.npz"
...
if __name__ == "__main__":
    ...
    main(args.ranks, args.q_vals, args.shard, args.n_shards, max_iter=args.max_iter)
